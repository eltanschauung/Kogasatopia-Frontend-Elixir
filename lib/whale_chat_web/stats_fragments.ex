defmodule WhaleChatWeb.StatsFragments do
  ...
  def focused_player_html(row, default_avatar) do
    ...
    <section class="detail-panel stats-focused-panel" data-focused-player="#{e(row[:steamid] || "")}">
      <div class="detail-profile">
        ...
        <div>
          ...
          <div style="color:var(--text-muted,#b7c0d2);">#{number(row[:kills])} K / #{number(row[:deaths])} D / #{number(row[:assists])} A</div>
        </div>
      </div>
      <div class="detail-grid">
        <div><h3>Damage</h3><p>#{number(row[:damage_dealt])}</p></div>
        <div><h3>DT</h3><p>#{number(row[:damage_taken])}</p></div>
        <div><h3>DPM</h3><p>#{decimal(dpm, 1)}</p></div>
        <div><h3>DT/M</h3><p>#{decimal(dtpm, 1)}</p></div>
        <div><h3>Accuracy</h3><p>#{decimal(acc, 1)}%</p></div>
        <div><h3>K/D</h3><p>#{decimal(kd, 2)}</p></div>
        <div><h3>Healing</h3><p>#{number(row[:healing])}</p></div>
        <div><h3>Headshots</h3><p>#{number(row[:headshots])}</p></div>
        <div><h3>Stabs</h3><p>#{number(row[:backstabs])}</p></div>
        <div><h3>Best Streak</h3><p>#{number(row[:best_killstreak])}</p></div>
        <div><h3>Ubers</h3><p>#{number(row[:total_ubers])}</p></div>
